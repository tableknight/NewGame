...
//        ]
    ...
    override func create() {
        ...
        addSingleCellItem(x: 0, line: 2, texture: Game.instance.outside_b.getCell(8, 7))
...
//        addSingleCellItem(x: 7, line: 4, texture: Data.instance.outside_b.getCell(5, 12, 1, 2))
        addSingleCellItem(x: 3, line: 3.5, texture: Game.instance.outside_c.getCell(2, 7, 3, 2))
        addSingleCellItem(x: 6, line: 2.35, texture: Game.instance.pictureActor3.getCell(7, 4), z: UIStage.LAYER2 + 2)
        ...
        addSingleCellItem(x: 11, line: 3.25, texture: Game.instance.sf_outside_c.getCell(0, 3, 3, 2))
        let vh = VillageHead()
//        vh.name = "pos14"
...
//        addSingleCellItem(x: 14, line: 2.5, texture: Data.instance.picturePeople1.getCell(7, 4), z: UIStage.LAYER2 + 1)
        addSingleCellItem(x: 14, line: 3, texture: Game.instance.sf_outside_c.getCell(3, 2, 2, 1))
...
//        wv.create()
        Game.instance.stage.switchScene(next: wv)
    }
    override func lastCellEvent() {
        let ev = EastOfNewComersVillage()
//        ev.create()
        Game.instance.stage.switchScene(next: ev)
    }
    private func testIberis() {
        let e = Game.instance._char!
        for _ in 0...63 {
            e.levelup()
        }
        //325
        e.staminaChange(value: 100)
        e.strengthChange(value: 150)
        e.agilityChange(value: 100)
        e.intellectChange(value: 75)
        for m in e._minions {
            m.levelTo(level: 63)
        }
        ...
        let boss = Iberis()
        boss.create(level: 63)
        es.append(boss)
        boss._seat = BUnit.RRM
...
//        b.setRoles(roles: roles)
    ...
    private func testFight() {
        let e = Game.instance._char!
        for _ in 0...42 {
            e.levelup()
        }
        //220
        e.staminaChange(value: 50)
        e.strengthChange(value: 100)
        e.agilityChange(value: 50)
        e.intellectChange(value: 20)
        for m in e._minions {
            m.levelTo(level: 44)
        }
        
        var es = Array<Creature>()
        let boss = DeathAngelKhalil()
        boss.create(level: 42)
        es.append(boss)
        boss._seat = BUnit.RRM
...
//        es.append(nc3)
//        
//        let lb = LoiteringBanshee()
//        lb.create(level: 42)
//        lb._seat = BUnit.RRT
//        es.append(lb)
//        
        let lb1 = LoiteringBanshee()
        lb1.create(level: 42)
        lb1._seat = BUnit.RRB
        es.append(lb1)
...
//            let item = SKSpriteNode(texture: Data.instance.outside_c.getCell(5, 7, 3, 2))
            let item = setItem(x: 3, line: 3.5, texture: Game.instance.outside_c.getCell(5, 7, 3, 2))
            item.alpha = 0
//            item.position.y = cellSize * 2
            ...
            item.run(q) {
                ...
                dlp.create()
                Game.instance.stage.addPanelDefault(p: dlp)
            }
        } else if 6 == pos {
            let _ = stage.showDialog(img: Game.instance.pictureActor3.getCell(7, 4),
...
//            })
    ...
    private func showRoadMessage() {
        ...
        rmp.create()
        Game.instance.stage.addPanelDefault(p: rmp)
    }
...
class Spring:UIItem {
    override init(texture: SKTexture?, color: UIColor, size: CGSize) {
        super.init(texture: texture, color: color, size: size)
        let n = Game.instance.sf_outside_c.getCell(0, 3, 3, 2)
        setTexture(n)
        _xSize = 3
        _ySize = 2
    }
...
class DungeonPortal:UIItem {
    override init(texture: SKTexture?, color: UIColor, size: CGSize) {
        super.init(texture: texture, color: color, size: size)
        let n = Game.instance.outside_c.getCell(2, 7, 3, 2)
        setTexture(n)
        _xSize = 3
        _ySize = 2
    }
...
class DungeonPortalLight:UIItem {
    override init(texture: SKTexture?, color: UIColor, size: CGSize) {
        super.init(texture: texture, color: color, size: size)
        let n = Game.instance.outside_c.getCell(5, 7, 3, 2)
        setTexture(n)
        _xSize = 3
        _ySize = 2
    }
...
//        _label = label
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if _closeButton.contains(touchPoint!) {
            Game.instance.stage.removePanelDefault(p: self)
...
//        let char = Data.instance._char!
        ...
        addChild(_listBox)
        _level = Game.instance._char._dungeonLevel
        createList()
    }
...
class FlowerRoad:UIItem {
    override init(texture: SKTexture?, color: UIColor, size: CGSize) {
        super.init(texture: texture, color: color, size: size)
        let n = Game.instance.tilee_outsidetown.getCell(14, 15, 1, 1)
...
class MudRoad:UIItem {
    override init(texture: SKTexture?, color: UIColor, size: CGSize) {
        super.init(texture: texture, color: color, size: size)
        let n = Game.instance.outside_b.getCell(4, 9, 1, 1)
        let node1 = SKSpriteNode(texture: n)
        node1.anchorPoint = CGPoint(x: 0.5, y: 0)
        let node2 = SKSpriteNode(texture: n)
        node2.anchorPoint = CGPoint(x: 0.5, y: 1)
...
class VillageHead:UIItem {
    override init(texture: SKTexture?, color: UIColor, size: CGSize) {
        super.init(texture: texture, color: color, size: size)
        setTexture(Game.instance.picturePeople1.getCell(7, 4))
        _xSize = 1
        _ySize = 1
    }
...
class DoubleGrass:UIItem {
    override init(texture: SKTexture?, color: UIColor, size: CGSize) {
        super.init(texture: texture, color: color, size: size)
        let item = Game.instance.tilee_outsidetown.getCell(7, 15, 2, 1)
        setTexture(item)
        _xSize = 2
        _ySize = 1
    }
