...
//            createBattle(evils:evil._evils)
        ...
        let mv = SKAction.moveBy(x: _destX > 0 ? -cellSize : cellSize, y: 0, duration: TimeInterval(Game.instance.frameSize * 2))
        let this = self
...
//    private var _currentX:CGFloat = 0
    private var _charButton:RoundButton!
    private var _minionButton:RoundButton!
    private var _spellButton:RoundButton!
    private var _itemButton:RoundButton!
//    private var _foreButton:RoundButton!
//    private var _afterButton:RoundButton!
//    private var _outfitButton:MenuButton!
    private var _logPanel:SKSpriteNode = SKSpriteNode()
    func createLogPanel() {
        _logPanel.zPosition = UIStage.UILAYER
        addChild(_logPanel)
        _uis.append(_logPanel)
//        let startY = -Game.instance.screenHeight * 0.5
//        _foreButton = createMenuButtons(x: -cellSize * 3.5 - 20, y: startY + 20, size: 20, text: "上")
//        _foreButton = createMenuButtons(x: -cellSize * 3.5 - 20, y: startY + 20, size: 20, text: "上")
        showLogs()
    }
    func showLogs() {
        _logPanel.removeAllChildren()
        let logs = getCurrentLogs()
        let startY = -Game.instance.screenHeight * 0.5 + 5
        let gap:CGFloat = 20
        for i in 0...logs.count - 1 {
            createLogNode(text: logs[i], y: startY + i.toFloat() * gap)
        }
    }
    func getCurrentLogs() -> Array<String> {
        return Game.instance.logs
    }
    func createLogNode(text:String, y:CGFloat) {
        let label = Label()
        label.align = "left"
        label.position.x = -cellSize * 3
        label.position.y = y
        label.fontSize = 14
        label.text = text
        label.color = UIColor.white
        _logPanel.addChild(label)
//        return label
    }
    func createMenu() {
//        let startX = Game.instance.screenWidth * 0.5 - cellSize * 0.5
//        let y = -Game.instance.screenHeight * 0.5 + cellSize * 0.5 - 5
...
//        _uis.append(_minionButton)
        ...
        _charButton = createMenuButtons(x: hw - 30, y: -hh + 30, size: 30, text: "角色")
        _itemButton = createMenuButtons(x: hw - 75, y: -hh + 75, size: 20, text: "物品")
//        _spellButton = createMenuButtons(x: hw - 140, y: -hh + 20, size: 20, text: "法术")
//        _minionButton = createMenuButtons(x: hw - 190, y: -hh + 20, size: 20, text: "随从")

        _spellButton = createMenuButtons(x: hw - 90, y: -hh + 20, size: 20, text: "法术")
        _minionButton = createMenuButtons(x: hw - 20, y: -hh + 90, size: 20, text: "随从")
...
//        _uis.append(_outfitButton)
    ...
    private var _actor:SKSpriteNode!
    private var _hpbar:HBar!
    private var _expbar:HBar!
    private var _levelLabel:Label!
    func createActor() {
        let c = Game.instance._char!
        let x = -Game.instance.screenWidth * 0.5
        let y = -Game.instance.screenHeight * 0.5
        let avator = SKSpriteNode(texture: c._img.getCell(1, 0))
        avator.position.x = x + cellSize * 0.5
        avator.position.y = y + cellSize
        addChild(avator)
        _actor = avator
        _uis.append(_actor)
        
        _hpbar = HBar()
        _hpbar.create(width: cellSize * 2, height: 6, value: 1, color: UIColor.red)
        _hpbar.position.x = x + cellSize
        _hpbar.position.y = y + 16
        addChild(_hpbar)
        _uis.append(_hpbar)
        
        _expbar = HBar()
        _expbar.create(width: cellSize * 2, height: 4, value: 1, color: UIColor.green)
        _expbar.position.x = x + cellSize
        _expbar.position.y = y + 8
        addChild(_expbar)
        _uis.append(_expbar)

        _levelLabel = Label()
        _levelLabel.position.x = x + cellSize
        _levelLabel.fontSize = 16
        _levelLabel.position.y = y + cellSize * 0.5
...
//        _uis.append(_levelLabel)
    ...
    func showUIPanel(panel:UIPanel) {
        hideUI()
        hideScene()
        addChild(panel)
    }
    func removeUIPanel(panel:UIPanel) {
        showUI()
        showScene()
        panel.removeFromParent()
    }
