...
class Scenery:Core {
    ...
    internal var cellSize = Game.instance.cellSize
...
//        }
        ...
        let mv = SKAction.move(by: cgv, duration: TimeInterval(Game.instance.frameSize * unit._moveSpeed))
        let __stop = seed(min:1, max:unit._moveSpeed.toInt() * 4).toFloat()
        let wait = SKAction.wait(forDuration: TimeInterval(Game.instance.frameSize * __stop))
