...
//                            char._money -= outfit._sellingPrice
                            if outfit.isRandom {
                                outfit.create(level: Game.instance._char._level)
                                outfit.isRandom = false
                            }
//                            outfit._price = outfit._quality
                            _lastSelectedItemComponent = SellingItemComponent()
                            pc.removeFromParent()
                            Game.instance._char.addProp(p: pc.prop)
                            filterSellableItems()
                            createSelfItemPanel()
                            return
...
//        createSellingItems()
        ...
        _moneyLabel.text = "\(Game.instance._char._money)G"
    ...
    func sell(sic:SellingItemComponent, count:Int) {
        ...
        let cp = Game.instance._char._props
...
//                _curPageItems.append(ac)
    ...
    private func showArmorProperties(armor:PropComponent) {
        ...
        if armor.position.x >= cellSize * 4 {
            ai.position.x = armor.position.x - ai._displayWidth - 25
        } else {
            ai.position.x = armor.position.x + cellSize * 0.5 + 5
        }
        if armor.position.y <= 0 {
            ai.position.y = ai._displayHeight - cellSize * 4 - armor.position.y
        } else {
            ai.position.y = ai._displayHeight - cellSize * 4 + armor.position.y
        }
        addChild(ai)
