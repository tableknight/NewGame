...
class RoleInfoPanel:SKSpriteNode {
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if _closeButton.contains(touchPoint!) {
            ...
            Game.instance.stage.closeCharPanel(panel: self)
            Game.instance.stage.showCharPanel(panel: ci)
            return
...
//        }
        ...
        let _hpbar = HBar()
        _hpbar.create(width: cellSize * 2, height: 6, value: char._extensions.hp / char._extensions.health, color: UIColor.red)
        _hpbar.position.y = lv.position.y - 9
        _hpbar.position.x = race.position.x + firstGap + cellSize
        addChild(_hpbar)
        
        let _expbar = HBar()
        _expbar.create(width: cellSize * 2, height: 6, value: char._exp / char.expNext(), color: UIColor.green)
        ...
        llt.position.x = (_expbar.position.x + cellSize) + 15
...
//                    _spellComponents.append(sc)
    ...
    private func close() {
        ...
        if !_unit.isMainChar {
            ...
            Game.instance.stage.showMinionsListPanel(panel: ml)
        }
    }
    ...
    private func setRoleSeat(seat:String, _ node:SKSpriteNode) {
        ...
        Game.instance._char._seat = seat
        
        for m in Game.instance._char._minions {
            if m._seat == seat {
